...
void setColorSphere(Vertex *v,float t,int k)
{
    ...
    for(u=0;u<1;u++){
        ...
        {
            if(intersectTriangle(p, l, &triangles[j]))
            {
                flag=true;
            }
        }
        if(flag==true)
        {
            ...
            v->color_diffuse[0] = v->color_diffuse[0]+ spheres[k].color_diffuse[0]*ln;
            v->color_diffuse[1] = v->color_diffuse[1]+ spheres[k].color_diffuse[1]*ln;
            v->color_diffuse[2] = v->color_diffuse[2]+ spheres[k].color_diffuse[2]*ln;
        }
    }
    if(flag1==true){
...
void  setColorTraingle(Vertex *v,float t,int k,int s)
{
    ...
    for(u=0;u<1;u++){
        ...
        if(flag==true)
        {
            ...
            //INTERPOLATION OF DIFFUSE COLOR
            v->color_diffuse[0] =v->color_diffuse[0] + (A1/A)*p0_diffuse[0]+(A2/A)*p1_diffuse[0]+(A3/A)*p2_diffuse[0];
            v->color_diffuse[1] =v->color_diffuse[1] + (A1/A)*p0_diffuse[1]+(A2/A)*p1_diffuse[1]+(A3/A)*p2_diffuse[1];
            v->color_diffuse[2] =v->color_diffuse[2] + (A1/A)*p0_diffuse[2]+(A2/A)*p1_diffuse[2]+(A3/A)*p2_diffuse[2];
...
void handleKeypress(unsigned char key, int x, int y)
{
    switch(key) {
        ...
        case 'p': lights[0].position[1]+=1.1;
            draw_scene();
            break;
